...
function validatePhoneNumber(phone) {
  ...
  if (!/^[6-9]\d{9}$/.test(phone)) return "Enter a valid Indian mobile number.";
  if (/^(\d)\1{9}$/.test(phone) || ["0123456789", "1234567890", "0987654321"].includes(phone)) {
...
function Settings() {
  ...
  const updateProfile = (field, value) => {
    ...
    if (field === "name") {
      setErrors((current) => ({ ...current, name: value.trim() ? "" : "Name is required." }));
    }
    if (field === "email") {
      setErrors((current) => ({ ...current, email: validateEmail(value.trim()) }));
    }
    if (field === "phone") {
      if (!/^\d*$/.test(value)) {
        setErrors((current) => ({ ...current, phone: "Phone number must contain digits only." }));
        return;
      }
      const phoneValue = value.slice(0, 10);
      setProfile((current) => ({ ...current, phone: phoneValue }));
      setErrors((current) => ({ ...current, phone: phoneValue ? validatePhoneNumber(phoneValue) : "" }));
    }
  };
  ...
  const saveProfile = () => {
    ...
    let existingUser = {};
    try {
      existingUser = JSON.parse(localStorage.getItem("spendwiseUser") || "{}");
    } catch (error) {
      console.error("Unable to read saved user:", error);
    }

    const updatedUser = { ...existingUser, name, email, phone };
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        <p className="text-xs font-medium uppercase tracking-[0.14em] text-slate-500 dark:text-white/35">
          Preferences
        </p>
      ...
      <Card>
        ...
        <div className="mt-2 divide-y divide-slate-200 dark:divide-white/[0.07]">
          <PreferenceRow
...
function PreferenceRow({ title, description, checked, onChange }) {
  return (
    <div className="flex items-center justify-between gap-5 py-5">
      <div>
        <p className="text-sm font-medium text-slate-800 dark:text-white/80">{title}</p>
        <p className="mt-1 max-w-xl text-xs leading-5 text-slate-500 dark:text-white/35">
          {description}
        </p>
      </div>
      <button
        ...
        className={`relative h-6 w-11 shrink-0 rounded-full transition ${
          checked ? "bg-[#d9a928]" : "bg-slate-300 dark:bg-white/[0.12]"
        }`}
      >
        <span
